...
static void
add_new_tcp(struct pcap_pkthdr *hdr, struct tcphdr * this_tcphdr, struct ip * this_iphdr)
{
  ...
  memcpy(&a_tcp->start_time, hdr, sizeof(struct pcap_pkthdr) );//add syn packet arrival time
...
int tcp_number = 0;
void
process_tcp(struct pcap_pkthdr *hdr, u_char * data, int skblen)
{
  ...
  tcp_number++;
    ...
    struct tm *tp;
	tp = localtime(&hdr->ts.tv_sec);

	 if(nids_params.fp_tcp){
	 fprintf(nids_params.fp_tcp, "process tcp number=%i, arrival time is %.04d%.02d%.02d %.02d:%.02d:%.02d.%.06u, caplen=%d, len=%d\n", 		tcp_number, tp->tm_year+1900, tp->tm_mon+1,
		   tp->tm_mday, tp->tm_hour, tp->tm_min, tp->tm_sec,
		   hdr->ts.tv_usec, hdr->caplen, hdr->len);
	 }
...
//ECN
     ...
     if(nids_params.fp_tcp)
 	 fprintf(nids_params.fp_tcp, "sip=%s,sport=%hu,dip=%s,dport=%hu\n",
 	 	int_ntoa(this_iphdr->ip_src.s_addr),
 		ntohs(this_tcphdr->th_sport),
 		int_ntoa(this_iphdr->ip_dst.s_addr),
 	 	ntohs(this_tcphdr->th_dport));

//三次握手中的第一次握手,判断谁是client,谁是server  在只有syn标志位中  sip是client, dip是server
  if (!(a_tcp = find_stream(this_tcphdr, this_iphdr, &from_client))) {
    if ((this_tcphdr->th_flags & TH_SYN) &&
	!(this_tcphdr->th_flags & TH_ACK) &&
	!(this_tcphdr->th_flags & TH_RST))
      add_new_tcp(hdr, this_tcphdr, this_iphdr);
    return;  //第一次握手完毕,直接返回
	
  }

  //从第二次握手开始,每一个数据流都需要判断clinet是snd或者是rcv
  //from_client 表示数据流方向 等于1表示client发送给server  等于0表示server发送给client
  if (from_client) {
    snd = &a_tcp->client;
    rcv = &a_tcp->server;
  }
  ...
  if ((this_tcphdr->th_flags & TH_SYN)) {
  	//满足下面任何一种情况,直接返回
  	//client发送给server    client状态不等于syn_SENT    server状态不等于close    flags中没有ack标志位
    if (from_client || a_tcp->client.state != TCP_SYN_SENT ||
      a_tcp->server.state != TCP_CLOSE || !(this_tcphdr->th_flags & TH_ACK))
      return;
	//服务端的ack确认第一次握手client的seq
  ...
  //第三次握手, 特点是 client发送给server,且只有ack标志位,数据长度为0
  //或则有传输数据 如果完全重叠了,直接返回
  if (
  	! (  !datalen && ntohl(this_tcphdr->th_seq) == rcv->ack_seq  )  //(第三次握手)数据长度为0, 而且client的seq等于前一次需要确认的seq
  	&&                                                              //(有数据在传输)数据长度大于0,继续后面判断
  	( !before(ntohl(this_tcphdr->th_seq), rcv->ack_seq + rcv->window*rcv->wscale) || 
  	             //上面这句是这次的数据seq大于已经确认的seq+窗口大小
          before(ntohl(this_tcphdr->th_seq) + datalen, rcv->ack_seq) 
          //上面这句是这次的数据完全重叠了之前已经确认的数据
        )
     )     
     return; //第三次握手完毕,直接返回
             //传输数据,但是完全重叠了,也直接返回,否则继续后面的处理
  ...
  if ((this_tcphdr->th_flags & TH_RST)) {
    if (a_tcp->nids_state == NIDS_DATA) {
	  ...
	  memcpy(&a_tcp->end_time, hdr, sizeof(struct pcap_pkthdr));  //设置最后一个包的arrival time
  ...
  if (datalen + (this_tcphdr->th_flags & TH_FIN) > 0)
    tcp_queue(a_tcp, this_tcphdr, snd, rcv,  //入队
