...
struct nids_prm nids_params = {
    304000,			/* n_tcp_streams */
    25600,			/* n_hosts */
    //"eth3",			/* device */
	NULL,
    "/mnt/pcap_shell/pcap/vmax.pcap",			/* filename */
	//NULL,
	168,			/* sk_buff_size */
    -1,				/* dev_addon */
    nids_syslog,		/* syslog() */
    ...
    "tcp or (vlan and tcp)",/* pcap_filter */
    1,				/* promisc */
    0,				/* one_loop_less */
    -1,			/* pcap_timeout */
    3,				/* multiproc */
...
static void nids_syslog(int type, int errnum, struct ip *iph, void *data)
{
    ...
    case NIDS_WARN_QUEUE:
    	//strcpy(saddr, int_ntoa(iph->ip_src.s_addr));
    	//strcpy(daddr, int_ntoa(iph->ip_dst.s_addr));

    	syslog(nids_params.syslog_level, "queue full, drop packet\n");
    			//   "queue full, drop packet [from %s:%hu to  %s:%hu]\n",
    			//   saddr, ntohs(((struct tcphdr *) data)->th_sport), daddr,
    			//   ntohs(((struct tcphdr *) data)->th_dport));
 ...
 */
static void call_ip_frag_procs(struct pcap_pkthdr *hdr, void *data,bpf_u_int32 caplen)
{
    struct proc_node *i;
    for (i = ip_frag_procs; i; i = i->next)
	(i->item) (hdr, data, caplen);
}
  ...
  data_aligned = data + nids_linkoffset;
     ...
     if(nids_params.multiproc) { 
        ...
        if (qitem && (qitem->data=malloc(hdr->caplen - nids_linkoffset))) {
		  ...
		  memcpy(&qitem->hdr, hdr, sizeof(struct pcap_pkthdr));
	    ...
	    free(qitem->data);
	    free(qitem);
        if(nids_params.syslog)
        		nids_params.syslog(NIDS_WARN_QUEUE, NIDS_WARN_QUEUE_FULL, (struct ip *)NULL,
			       (void *)NULL);
	    } else {
	    /* insert packet to queue */
	   // fprintf(stdout, "before insert queue. queue number =[%d]\n", g_async_queue_length_unlocked(cap_queue));
	    pushcount++;
	    g_async_queue_push_unlocked(cap_queue,qitem);
	    fprintf(stdout, "push queue count %d\n", pushcount);
	   // fprintf(stdout, "after insert queue. queue number =[%d]\n\n",g_async_queue_length_unlocked(cap_queue) );
         }
          g_async_queue_unlock(cap_queue);
	}
     } else { /* user requested simple passthru - no threading */
        call_ip_frag_procs(hdr, data_aligned,hdr->caplen - nids_linkoffset);
     }
 #else
     call_ip_frag_procs(hdr, data_aligned,hdr->caplen - nids_linkoffset);
 #endif
...
static void gen_ip_frag_proc(struct pcap_pkthdr *hdr, u_char * data, int len)
{
    struct proc_node *i;
...
static void gen_ip_proc(struct pcap_pkthdr *hdr, u_char * data, int skblen)
{
    switch (((struct ip *) data)->ip_p) {
    case IPPROTO_TCP:
	process_tcp(hdr, data, skblen);
	break;
    case IPPROTO_UDP:
...
#define START_CAP_QUEUE_PROCESS_THREAD() \
    if(nids_params.multiproc) { /* threading... */ \
	 if(!(g_thread_create_full((GThreadFunc)cap_queue_process_thread,NULL,0,FALSE,TRUE,G_THREAD_PRIORITY_HIGH,&gerror))) { \
...
static void cap_queue_process_thread()
{
   ...
   //  fprintf(stdout, "before pop queue. queue number =[%d]\n", g_async_queue_length(cap_queue));
	 qitem=g_async_queue_pop(cap_queue);
	 popcount++;
   // fprintf(stdout, "after pop queue. queue number =[%d]\n\n",g_async_queue_length(cap_queue) );
	 fprintf(stdout, "pop queue count %d\n", popcount);
	  if (qitem==&EOF_item) break; /* EOF item received: we should exit */
	  call_ip_frag_procs(&qitem->hdr, qitem->data,qitem->caplen);
	  free(qitem->data);
	  free(qitem);
     }
     g_thread_exit(NULL);
...
int nids_init()
{
    ...
    fprintf(stdout, "aysnc queue max length is %d\n", G_MAXINT);
...
#define DLT_LINUX_SLL   113
    ...
    if(nids_params.logfile_tcp){
	FILE *fp ;
	fp = fopen(nids_params.logfile_tcp, "w");
	if(fp == NULL){
		fprintf(stderr, "open log file fialed.\n");	
	}else{
		nids_params.fp_tcp = fp;
	}
    }
    if(nids_params.multiproc) {
#ifdef HAVE_LIBGTHREAD_2_0
...
#endif
    if(nids_params.fp_tcp){
	fclose(nids_params.fp_tcp);
	nids_params.fp_tcp = NULL;
    }
    tcp_exit();
    ip_frag_exit();
    scan_exit();
